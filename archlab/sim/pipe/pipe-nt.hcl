...
## This is the "select pc" logic.
### Is UNCOND within the IJXX set?
word f_pc = [
	# Mispredicted branch.  Fetch at incremented PC
	# Mispredicted branch when jump IS taken.
	# Check M_ifun, to see if uncond jmp. Fall thru to predPC.
...
## Pass E_valC into M_valA when conditional jumps.
word e_valA = [
...
bool D_bubble =
	# Mispredicted branch
	## No bubbles if uncond.jmp.
	!(E_ifun == UNCOND) &&
	## Interpret a "jump taken" case as a misprediction.
	(E_icode == IJXX && e_Cnd ) ||
	# Stalling at fetch while ret passes through pipeline
